...
//		}
        ...
        for (int j = 0; j < lotNoList.size(); j++) {
            ...
            /**
             * sql：getSum 的 sql语句，根据NG ID计算B和C列字段的和，如果>=1则视为实际NG
             * sql2：getGroupNgCount 的 sql语句，根据实际NG ID 计算主要缺陷字段中各字段的和
             * sql3：getGroupNgCount 的 sql语句，根据实际NG ID 计算主要缺陷字段和次要缺陷字段中各字段的和
             */
            StringBuffer sql = new StringBuffer();
...
//				}
                ...
                if (!group.equals("A")) {
                    ...
                    sql3.append("sum(" + columnName + ") as " + alias);
                }

                if (i != columnDetailList.size() - 1 && !group.equals("A")) {
                    sql.append(" + ");
                    if (group.equals("B")) {
                        sql2.append(" , ");
                    }
            ...
            sql.append(") as `sum` from " + table + " where ID = ");
            // 删除sql2最后一个","
            String substring = sql2.substring(0, sql2.lastIndexOf(","));
            substring += " from " + table + " where ID IN";
            sql2.append(" from " + table + " where ID IN");

            sql3.append(" from " + table + " where ID IN");
            ...
            if (!CollectionUtils.isEmpty(actualNgIds)) {
                map = sqliteMapper.t_getGroupNgCount(sql3.toString(), actualNgIds);
                mainMap = sqliteMapper.t_getGroupNgCount(substring, actualNgIds);
                ...
                for (String key : keys) {
                    BigDecimal value = (BigDecimal) mainMap.get(key);
                    total += value.intValue();
                }
                //String value = a.intValue() > 0 ? "Failed" : "OK";
                String result = total > 0 ? "Failed" : "OK";
    ...
    public AjaxResult getDatagrid(String date, String table) {
        ...
        str.append(
                "<table id=\"dg\" class=\"easyui-datagrid\"" +
                        ...
                        "<th data-options=\"field:'actualNgCount',align:'center',width:'100'\" rowspan=\"2\">NG</th>" +
                        //"<th>Total</th>" +
                        // B组长度 + C组长度
                        //"<th colspan=\"" + (groupB.size() + groupC.size()) + "\">NG</th>" +
                        //"</tr>" +
                        //"<tr>" +
                        "<th colspan=\"" + groupB.size() + "\">Major defect</th>" +
                        "<th colspan=\"" + groupC.size() + "\">Minor defect</th>" +
                        "</tr>" +
                        "<tr>");
