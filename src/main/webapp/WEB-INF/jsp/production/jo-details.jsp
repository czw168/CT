<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	<div id="joEditWindow" class="easyui-window" title="JO Edit" data-options="closed:true,iconCls:'icon-save',href:'${pageContext.request.contextPath }/production/jo-edit'" style="width:600px;height:500px;padding:10px;" ></div>
			...
			$('#joDetailsList').datagrid({
				...
				toolbar: [{
			        ...
			        handler:function(){
			        	$("#joAddWindow").window({
			        		onLoad :function(){
			        			var row = $('#joDetailsList').datagrid('getData').rows[0];
			        			$('#joAdd_uom').textbox('textbox').css('background','#ccc');
			        			$("#joAdd_ubotJo").val(row.ubotJo);
			        		}
			        	}).window("open");
			        }
			    }, {
			        ...
			        iconCls:'icon-edit',
			        handler:function(){
				    	var sel = $("#joDetailsList").datagrid("getSelections");
			        	if(sel.length == 0){
			        	...
			        	$("#joEditWindow").window({
			        		onLoad :function(){
			        			$('#joEdit_desc').textbox('textbox').css('background','#ccc');
				        		$('#joEdit_custPn').textbox('textbox').css('background','#ccc');
			        			// 获取所选行数据
			        			var data = $("#joDetailsList").datagrid("getSelections")[0];
			        			//回显数据
			        			$("#joEditFrom").form("load",data);
			        			$('#joEdit_updateBy').val("${subject.principal}");
			        		}
			        	}).window("open");
			        }
			    },{text:'<span style="margin-left: 550px;color: red;font-weight: bold;">JO NO. : <span id="joNo_details">'+joNo+'</span></span>'}]
			});
		}
